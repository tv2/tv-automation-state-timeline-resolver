...
export class AtemDevice extends DeviceWithState<DeviceState> {
	...
	handleState (newState: TimelineState) {
		...
		let previousState = Math.max(this.getCurrentTime(), newState.time)
		let oldState: DeviceState = (this.getStateBefore(previousState) || { state: this._getDefaultState() }).state
		...
		this._doOnTime.clearQueueNowAndAfter(previousState)
