...
class QuantelDevice extends device_1.DeviceWithState {
    ...
    get deviceType() {
        return src_1.DeviceType.QUANTEL;
    }
    get deviceName() {
            ...
            if (mapping &&
                ...
                if (!ports[qMapping.portId]) {
                    ports[qMapping.portId] = {
                        mode: qMapping.mode || src_1.QuantelControlMode.QUALITY,
                        channels: []
                    };
                }
    ...
    convertStateToQuantel(timelineState, mappings) {
        ...
        _.each(timelineState.layers, (layer, layerName) => {
            ...
            if (foundMapping &&
                foundMapping.device === src_1.DeviceType.QUANTEL &&
                foundMapping.deviceId === this.deviceId &&
                _.has(foundMapping, 'portId') &&
                _.has(foundMapping, 'channelId')) {
                const mapping = foundMapping;
                const port = state.port[mapping.portId];
                if (!port)
                    throw new Error(`Port "${mapping.portId}" not found`);
    ...
    _addToQueue(commandsToAchieveState) {
        _.each(commandsToAchieveState, (cmd) => {
            this._doOnTime.queue(cmd.time, cmd.portId, (c) => {
            ...
            this._doOnTimeBurst.queue(cmd.time, undefined, (c) => {
                if ((c.cmd.type === QuantelCommandType.PLAYCLIP ||
...
class QuantelManager extends events_1.EventEmitter {
    ...
    async prepareClipJump(cmd, alsoDoAction) {
        ...
        if ((jumpToOffset === trackedPort.offset &&
