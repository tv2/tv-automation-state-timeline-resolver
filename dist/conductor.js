    ...
    addDevice(deviceId, deviceOptions) {
        try {
            ...
            if (deviceOptions.type === src_1.DeviceType.ABSTRACT) {
                // Add Abstract device:
                newDevice = new abstract_1.AbstractDevice(deviceId, deviceOptions, options);
            }
                ...
                newDevice = new lawo_1.LawoDevice(deviceId, deviceOptions, options);
            }
            else if (deviceOptions.type === src_1.DeviceType.PANASONIC_PTZ) {
                newDevice = new panasonicPTZ_1.PanasonicPtzDevice(deviceId, deviceOptions, options);
            }
            else if (deviceOptions.type === src_1.DeviceType.HYPERDECK) {
                newDevice = new hyperdeck_1.HyperdeckDevice(deviceId, deviceOptions, options, this);
            }
            else if (deviceOptions.type === src_1.DeviceType.PHAROS) {
                newDevice = new pharos_1.PharosDevice(deviceId, deviceOptions, options);
            }
            else if (deviceOptions.type === src_1.DeviceType.OSC) {
                newDevice = new osc_1.OSCMessageDevice(deviceId, deviceOptions, options);
            }
            else {
                return Promise.reject('No matching device type for "' + deviceOptions.type + '" ("' + src_1.DeviceType[deviceOptions.type] + '") found');
            }
            ...
            this.emit('info', 'Initializing ' + src_1.DeviceType[deviceOptions.type] + '...');
            this.devices[deviceId] = newDevice;
            newDevice.mapping = this.mapping;
            return newDevice.init(deviceOptions.options)
                .then(() => {
                this.emit('info', (src_1.DeviceType[deviceOptions.type] + ' initialized!'));
                return newDevice;
            });
        }
        catch (e) {
